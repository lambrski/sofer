(function(){
  ...
  async function renderSynopsisCards(synopsisText) {
      ...
      cardView.querySelectorAll('.view-outline-btn').forEach(btn => btn.addEventListener('click', viewSavedOutline));
  }

  async function viewSavedOutline(e) {
    const btn = e.target;
    const chapterTitle = btn.getAttribute('data-chapter-title');
    btn.textContent = "טוען...";
    btn.disabled = true;
    try {
        const res = await fetch(`/api/project/${pid}/outline?chapter_title=${encodeURIComponent(chapterTitle)}`);
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Could not load outline.");
        
        currentOutlineText = data.outline_text;
  ...
  async function writeChapterFromCard(e) {
      ...
      try {
          ...
          if (pkind === 'פרוזה') {
            currentOutlineText = data.answer; 
            currentOutlineChapterTitle = chapterTitle;
  ...
  function closeDraftAndViewOutline() {
      document.getElementById('chapterOutputModal').style.display = 'none';
      const outlineModal = document.getElementById('outlineManagerModal');
      if (outlineModal) {
  ...
  safeAttach('closeChapterOutputBtn', 'click', closeDraftAndViewOutline);
  ...
  document.addEventListener("keydown", (ev)=>{
    if (ev.ctrlKey && ev.key==="Enter"){
      ...
      if (activeEl.id === 'sceneDiscussionInput') { document.getElementById('sendSceneDiscussionBtn').click(); ev.preventDefault(); }
      if (activeEl.id === 'draftDiscussionInput') { document.getElementById('sendDraftDiscussionBtn').click(); ev.preventDefault(); }
  ...
  // --- Outline Manager Logic ---
  function openOutlineManager(outlineText, chapterTitle) {
    const modal = document.getElementById('outlineManagerModal');
    const titleEl = document.getElementById('outlineManagerTitle');
    const contentEl = document.getElementById('outlineManagerContent');
    const saveBtn = document.getElementById('saveOutlineBtn');
    
    saveBtn.setAttribute('data-chapter-title', chapterTitle);
    titleEl.textContent = `מתווה עבור: ${esc(chapterTitle)}`;
    contentEl.innerHTML = ''; // Clear previous content
    
    const potentialBlocks = outlineText.split(/\n\s*\n/); 
    const sceneBlocks = potentialBlocks.filter(block => block.includes("סצנה"));

    if (sceneBlocks.length === 0) {
        contentEl.innerHTML = `<div class='muted'>לא נמצאו סצנות תקינות במתווה.</div>`;
    } else {
        let cardIdCounter = 0;
        const cardsHtml = sceneBlocks.map(sceneText => {
            ...
            const titleEndMatch = firstLine.match(/\*\*:/) || firstLine.match(/##:/) || firstLine.match(/#:/);
            let splitIndex = -1;
            if (titleEndMatch) {
                splitIndex = firstLine.indexOf(titleEndMatch[0]) + titleEndMatch[0].length;
            }

            if (splitIndex > -1) {
                sceneTitle = firstLine.substring(0, splitIndex).trim();
                let firstLineContent = firstLine.substring(splitIndex).trim();
                sceneContent = (firstLineContent + '\n' + lines.join('\n')).trim();
            } else {
                sceneTitle = firstLine;
                sceneContent = lines.join('\n').trim();
            }
            
            const cardId = `scene-card-content-${cardIdCounter++}`;

            return `
              <div class="outline-card">
                  <h5>${esc(sceneTitle)}</h5>
        ...
        contentEl.querySelectorAll('.discuss-scene-btn').forEach(btn => btn.addEventListener('click', openSceneDiscussion));
        contentEl.querySelectorAll('.write-scene-btn').forEach(btn => btn.addEventListener('click', writeSceneDraft));
    }
  ...
  safeAttach('saveOutlineBtn', 'click', async (e) => {
    const btn = e.target;
    const chapterTitle = btn.getAttribute('data-chapter-title');
    if (!chapterTitle) {
        alert("שגיאה: לא נמצאה כותרת פרק לשמירה.");
        return;
    }
    btn.disabled = true;
    ...
    try {
        const body = new URLSearchParams({
            ...
            outline_text: currentOutlineText
        });
        const res = await fetch(`/api/project/${pid}/outline`, { method: "POST", body });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "שגיאת שרת");
        ...
        const synopsisCardView = document.getElementById('synopsisCardView');
        if (synopsisCardView.style.display === 'block') {
            renderSynopsisCards(document.getElementById('synopsisArea').value);
        }
  ...
  function openSceneDiscussion(e) {
    const btn = e.target;
    const cardId = btn.getAttribute('data-card-id');
    const sceneTitle = btn.getAttribute('data-scene-title');
    const sceneContentEl = document.getElementById(cardId);
    if (!sceneContentEl) return;

    currentSceneDiscussion = {
        cardId: cardId,
        title: sceneTitle,
        originalContent: sceneContentEl.textContent,
        thread: [],
        fullOutline: currentOutlineText
    };

    const modal = document.getElementById('sceneDiscussionModal');
    document.getElementById('sceneDiscussionTitle').textContent = `דיון על: ${esc(sceneTitle)}`;
    document.getElementById('sceneDiscussionThread').innerHTML = '<div class="muted">הדיון ריק. שאל שאלה כדי להתחיל.</div>';
    document.getElementById('sceneDiscussionInput').value = "";
    openModal(modal);
  }
  safeAttach('closeSceneDiscussionBtn', 'click', closeAllModals);

  safeAttach('sendSceneDiscussionBtn', 'click', async () => {
    const input = document.getElementById('sceneDiscussionInput');
    const question = input.value.trim();
    if (!question) return;

    const btn = document.getElementById('sendSceneDiscussionBtn');
    const threadEl = document.getElementById('sceneDiscussionThread');
    const originalText = btn.textContent;
    
    if(currentSceneDiscussion.thread.length === 0) { threadEl.innerHTML = ""; }
    threadEl.innerHTML += `<div class="turn q"><div class="bubble">${esc(question)}</div></div>`;
    threadEl.scrollTop = threadEl.scrollHeight;
    currentSceneDiscussion.thread.push({role: 'user', content: question});
    input.value = "";
    btn.disabled = true;
    btn.innerHTML = `<div class='spinner' style="width:12px; height:12px; border-width:2px;"></div>`;
    ...
    try {
        const body = new URLSearchParams({ 
            text: question,
            chapter_content: currentSceneDiscussion.originalContent,
            full_synopsis: currentSceneDiscussion.fullOutline, 
            discussion_thread: JSON.stringify(currentSceneDiscussion.thread),
            mode: 'brainstorm',
            write_kind: 'chat'
        });
        ...
        threadEl.innerHTML += `<div class="turn a"><div class="bubble">${esc(data.answer)}</div></div>`;
        currentSceneDiscussion.thread.push({role: 'assistant', content: data.answer});
        threadEl.scrollTop = threadEl.scrollHeight;
    } catch (err) {
        threadEl.innerHTML += `<div class="turn a"><div class="bubble" style="color:red;">שגיאה: ${esc(err.message)}</div></div>`;
    } finally {
  ...
  safeAttach('summarizeSceneBtn', 'click', async () => {
    if (currentSceneDiscussion.thread.length === 0) {
        alert("לא ניתן לסכם דיון ריק.");
        return;
    }
    const btn = document.getElementById('summarizeSceneBtn');
    const originalText = btn.textContent;
    btn.disabled = true;
    btn.innerHTML = `<div class='spinner' style="width:12px; height:12px; border-width:2px;"></div>`;
    ...
    try {
        const body = new URLSearchParams({
            original_content: currentSceneDiscussion.originalContent,
            discussion_thread: JSON.stringify(currentSceneDiscussion.thread),
            chapter_outline: currentSceneDiscussion.fullOutline
        });
        const res = await fetch(`/api/project/${pid}/update_scene_from_discussion`, { method: "POST", body });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "שגיאת שרת");

        const sceneContentEl = document.getElementById(currentSceneDiscussion.cardId);
        if (sceneContentEl) {
            sceneContentEl.textContent = data.updated_content;
            alert("תיאור הסצנה עודכן!");
            closeAllModals();
        }
    } catch(err) {
        alert("שגיאה בעדכון הסצנה: " + err.message);
    } finally {
        btn.disabled = false;
        btn.innerHTML = originalText;
    }
  ...
  async function writeSceneDraft(e) {
    const btn = e.target;
    const cardId = btn.getAttribute('data-card-id');
    const sceneTitle = btn.getAttribute('data-scene-title');
    const sceneContentEl = document.getElementById(cardId);
    ...
    if (!confirm(`האם לכתוב טיוטה עבור: "${sceneTitle}"?`)) return;
    ...
    try {
        const body = new URLSearchParams({
            ...
            scene_description: sceneContentEl.textContent
        });
        const res = await fetch(`/api/project/${pid}/write_scene`, { method: "POST", body });
        const data = await res.json();
        
        if (!res.ok) throw new Error(data.error || "שגיאת שרת");
        
        const chapterModal = document.getElementById('chapterOutputModal');
        const chapterOutputTitle = document.getElementById('chapterOutputTitle');
        const chapterOutputContent = document.getElementById('chapterOutputContent');
        
        if (chapterModal && chapterOutputTitle && chapterOutputContent) {
            currentDraftDiscussion.sceneDescription = sceneContentEl.textContent;
            
            chapterOutputTitle.textContent = `טיוטת סצנה: ${sceneTitle}`;
            chapterOutputContent.innerHTML = esc(data.scene_draft).replace(/\n/g, '<br>');
            document.getElementById('outlineManagerModal').style.display = 'none';
            openModal(chapterModal);
        } else {
            throw new Error("לא נמצאו רכיבי תצוגה חיוניים.");
        }
        
    } catch(err) {
        alert("שגיאה בכתיבת טיוטת הסצנה: " + err.message);
    } finally {
        btn.disabled = false;
        btn.textContent = "✍️ כתוב טיוטה לסצנה";
    }
  }
  
  // --- Draft Discussion Logic ---
  safeAttach('discussDraftBtn', 'click', (e) => {
    const draftContentEl = document.getElementById('chapterOutputContent');
    const editor = draftContentEl.querySelector('textarea');
    const originalDraft = editor ? editor.value : draftContentEl.innerHTML.replace(/<br\s*[\/]?>/gi, "\n");
    
    currentDraftDiscussion = {
        sceneDescription: currentDraftDiscussion.sceneDescription,
        originalDraft: originalDraft,
        thread: []
    };

    const modal = document.getElementById('draftDiscussionModal');
    document.getElementById('draftDiscussionTitle').textContent = `דיון על טיוטת הסצנה`;
    document.getElementById('draftDiscussionThread').innerHTML = '<div class="muted">הדיון ריק. שאל שאלה כדי להתחיל.</div>';
    document.getElementById('draftDiscussionInput').value = "";
    document.getElementById('draftDiscussionEditor').value = originalDraft;
    
    document.getElementById('chapterOutputModal').style.display = 'none';
    openModal(modal);
  });
  
  safeAttach('closeDraftDiscussionBtn', 'click', () => {
    const updatedDraft = document.getElementById('draftDiscussionEditor').value;
    const draftContentEl = document.getElementById('chapterOutputContent');
    const editor = draftContentEl.querySelector('textarea');
    if (editor) {
        editor.value = updatedDraft;
    } else {
        draftContentEl.innerHTML = esc(updatedDraft).replace(/\n/g, '<br>');
    }

    document.getElementById('draftDiscussionModal').style.display = 'none';
    document.getElementById('chapterOutputModal').style.display = 'block';
  });

  safeAttach('sendDraftDiscussionBtn', 'click', async () => {
    const input = document.getElementById('draftDiscussionInput');
    const question = input.value.trim();
    if (!question) return;

    const btn = document.getElementById('sendDraftDiscussionBtn');
    const threadEl = document.getElementById('draftDiscussionThread');
    const originalText = btn.textContent;
    
    if(currentDraftDiscussion.thread.length === 0) { threadEl.innerHTML = ""; }
    threadEl.innerHTML += `<div class="turn q"><div class="bubble">${esc(question)}</div></div>`;
    threadEl.scrollTop = threadEl.scrollHeight;
    currentDraftDiscussion.thread.push({role: 'user', content: question});
    input.value = "";
    btn.disabled = true;
    btn.innerHTML = `<div class='spinner' style="width:12px; height:12px; border-width:2px;"></div>`;
    ...
    try {
        const body = new URLSearchParams({ 
            text: question,
            original_draft: document.getElementById('draftDiscussionEditor').value,
            scene_description: currentDraftDiscussion.sceneDescription,
            discussion_thread: JSON.stringify(currentDraftDiscussion.thread),
            mode: 'brainstorm',
            write_kind: 'chat'
        });
        ...
        threadEl.innerHTML += `<div class="turn a"><div class="bubble">${esc(data.answer)}</div></div>`;
        currentDraftDiscussion.thread.push({role: 'assistant', content: data.answer});
        threadEl.scrollTop = threadEl.scrollHeight;
    } catch (err) {
        threadEl.innerHTML += `<div class="turn a"><div class="bubble" style="color:red;">שגיאה: ${esc(err.message)}</div></div>`;
    } finally {
  ...
  safeAttach('summarizeDraftBtn', 'click', async () => {
    if (currentDraftDiscussion.thread.length === 0) {
        alert("לא ניתן לסכם דיון ריק.");
        return;
    }
    const btn = document.getElementById('summarizeDraftBtn');
    const originalText = btn.textContent;
    btn.disabled = true;
    btn.innerHTML = `<div class='spinner' style="width:12px; height:12px; border-width:2px;"></div>`;
    ...
    try {
        ...
        const res = await fetch(`/api/project/${pid}/update_draft_from_discussion`, { method: "POST", body });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "שגיאת שרת");

        const draftEditor = document.getElementById('draftDiscussionEditor');
        draftEditor.value = data.updated_draft;
        alert("הטיוטה עודכנה!");
