...
def ask_project(
    ...
    with Session(engine) as session:
        ...
        full_context = ""
        is_discussion = False
        # Chapter discussion call
        if full_synopsis and chapter_content and discussion_thread:
            is_discussion = True
            thread_data = json.loads(discussion_thread)
            thread_str = "\n".join([f"{t['role']}: {t['content']}" for t in thread_data])
            full_context = f"**Full Synopsis Context:**\n{full_synopsis}\n\n**Original Chapter Content (Focus):**\n{chapter_content}\n\n**Current Discussion:**\n{thread_str}"
