# -*- coding: utf-8 -*-
# FINAL, COMPLETE, AND CORRECTED CODE (V37 - Style-First Prompt Implemented)
...
"""
        ...
        if project.kind == 'קומיקס' and write_kind == 'breakdown_chapter':
            ...
            prompt = f"{preamble}{full_context}להלן תקציר של פרק בקומיקס:\n---\n{chapter_synopsis}\n---\nהמשימה שלך היא לכתוב את התסריט המפורט עבור הפרק. עליך לפרק את התקציר לעמודים ופריימים, לפי מבנה של {pages_per_chapter:.1f} עמודים, ו-{project.frames_per_page or 6} פריימים לעמוד. התחל מ'עמוד 1, פריים 1'. הקפד על הפורמט: מספר פריים, אחריו טקסט (אם יש הרהור, ציין 'הרהור:'), ובשורה נפרדת תיאור ויזואלי בסוגריים מרובעים.\nדוגמה: 1.\nהרהור: עוד הפרעה...\n[קצין במדים יושב במשרד מפואר ומביט בטלפון המצלצל]."
...
def create_object(project_id: int, name: str = Form(...), description: str = Form(...), style: str = Form("")):
    ...
    # NEW: Style is now at the beginning of the prompt.
...
def create_image(project_id: int, desc: str = Form(...), style: str = Form(""), scene_label: str = Form(""), source_image_id: Optional[int] = Form(None)):
    with Session(engine) as session:
        try:
            ...
            # OLD
            # raw_prompt_text = f"A full scene image. Description: {english_desc}"
            # if style: raw_prompt_text += f", in the style of {style}"
