# -*- coding: utf-8 -*-
# FINAL, COMPLETE, AND REBUILT CODE (V48)
...
# ====== Image Generation Function (Using New GenAI SDK) ======
...
def rewrite_prompt_for_image_generation(raw_prompt: str) -> str:
    ...
    Instead of using potentially sensitive words directly (e.g., 'child', 'boy'), describe the subject's apparent age and features (e.g., 'a young person with youthful features', 'a figure wearing traditional religious attire').
    ...
    """
    try:
        response = text_model.generate_content(meta_prompt)
        rewritten_prompt = response.text.strip()
        print(f"Rewritten prompt: '{rewritten_prompt}'")
        return rewritten_prompt
    except Exception as e:
        print(f"Error during prompt rewrite: {e}")
        # Raise a more specific exception to be caught by the calling function
        raise RuntimeError(f"Prompt rewriting failed. This may be due to safety filters. Original error: {e}") from e
...
<body data-project-id="{{project.id}}" data-project-kind="{{project.kind}}">
  ...
  <div id="chapterOutputModal" class="modal">
    <header>
        <strong id="chapterOutputTitle">תוצר פרק</strong>
        <button id="closeChapterOutputBtn" class="linklike">סגור</button>
    </header>
    <div class="content">
        <div id="chapterOutputContent"></div>
    </div>
    <footer>
        <button id="editChapterBtn" class="linklike">✍️ ערוך</button>
        <button id="saveChapterEditsBtn" class="linklike" style="display:none;">💾 שמור שינויים</button>
        <button id="appendChapterBtn" class="linklike">הוסף לקובץ כללי  Append ➡️</button>
    </footer>
  </div>
...
(function(){
  ...
  function renderSynopsisCards(synopsisText) {
      ...
      fetch(`/api/project/${pid}/parse_synopsis`, {
          ...
          cardView.innerHTML = data.chapters.map(chap => {
              const chapterNumberMatch = chap.title.match(/[0-9]+/);
              ...
              <div class="chapter-card">
                  ...
                  <button class="linklike write-chapter-btn" data-content="${esc(chap.title)}" data-chapter-number="${chapterNumber}">✍️ כתוב את הפרק</button>
              </div>
          `}).join('');
  ...
  async function writeChapterFromCard(e) {
      ...
      const chapterNumber = e.target.getAttribute('data-chapter-number');
      if (!confirm(`האם לכתוב את הפרק: "${content}"?`)) return;
      closeAllModals();
      
      const status = document.getElementById('status');
      status.innerHTML = `<div class="spinner"></div> <span>כותב את ${content}...</span>`;
      try {
          ...
          document.getElementById('chapterOutputTitle').textContent = `תוצר: ${content}`;
  ...
  safeAttach('divideSynopsisBtn', 'click', async () => {
    ...
    if (!confirm("פעולה זו תשלח את התקציר למודל לחלוקה לפרקים. התוצאה תחליף את הטקסט הנוכחי בעורך. האם להמשיך?")) return;
  ...
  // NEW: Logic for the Chapter Output Modal
  safeAttach('closeChapterOutputBtn', 'click', closeAllModals);
  ...
  safeAttach('saveChapterEditsBtn', 'click', (e) => {
    ...
    contentDiv.innerHTML = esc(newText).replace(/\\n/g, '<br>');
  ...
  safeAttach('appendChapterBtn', 'click', async (e) => {
    ...
    try {
      // 1. Get current chapter text from the modal
      const contentDiv = document.getElementById('chapterOutputContent');
      let chapterText = '';
      if (contentDiv.querySelector('textarea')) {
        chapterText = contentDiv.querySelector('textarea').value;
      } else {
        chapterText = contentDiv.textContent;
      }

      // 2. Fetch the existing text from General Notes
      const res = await fetch("/general/"+pid);
      if (!res.ok) throw new Error('Failed to fetch general notes.');
      const data = await res.json();
      const existingNotes = data.text || "";

      // 3. Combine the texts
      const newNotes = existingNotes + "\n\n---\n\n" + chapterText;

      // 4. Send the new combined text back to the server
      ...
      alert("שגיאה בהוספה לקובץ כללי: " + err.message);
    } finally {
      btn.disabled = false;
      btn.textContent = 'הוסף לקובץ כללי ➡️';
    }
  });
...
def save_synopsis(project_id: int, text: str = Form("")):
    with Session(engine) as session:
        proj = session.get(Project, project_id)
        if proj:
            # Only save to history if the text has actually changed and is not empty
            if text.strip() and proj.synopsis_text != text:
                history_entry = SynopsisHistory(project_id=project_id, text=text)
                session.add(history_entry)
            
            # Always update the main project table
...
def ask_project(project_id: int, text: str = Form(""), use_notes: str = Form("1"), mode: str = Form(...), write_kind: str = Form(...), use_history: str = Form("1"), temperature: float = Form(0.7), persona: str = Form("partner"), temp_file_ids: List[str] = Form([]), library_file_ids: List[int] = Form([]), synopsis_text_content: Optional[str] = Form(None), chapter_number: Optional[int] = Form(None)):
    with Session(engine) as session:
        project = session.get(Project, project_id)
        if not project:
...
"""
        ...
        if mode == 'brainstorm' or mode == 'write':
            if persona == 'assistant':
                ...
                preamble += "הפרסונה שלך היא 'שותף יצריתי מקצועי'...\n\n"
        ...
        if project.kind == 'קומיקס' and write_kind == 'breakdown_chapter':
            ...
            current_chapter_num = chapter_number or 1
            start_page = ((current_chapter_num - 1) * pages_per_chapter) + 1
            start_frame = ((current_chapter_num - 1) * pages_per_chapter * frames_per_page) + 1
            instruction = f"התחל את מספור העמודים מ'עמוד {start_page}' ואת מספור הפריימים מ'פריים {start_frame}'."

            prompt = f"{preamble}{full_context}להלן תקציר של פרק בקומיקס:\n---\n{chapter_synopsis}\n---\nהמשימה שלך היא לכתוב את התסריט המפורט עבור הפרק. עליך לפרק את התקציר לעמודים ופריימים, לפי מבנה של {pages_per_chapter} עמודים, ו-{frames_per_page} פריימים לעמוד. {instruction} הקפד על הפורמט: מספר פריים, אחריו טקסט (אם יש הרהור, ציין 'הרהור:'), ובשורה נפרדת תיאור ויזואלי בסוגריים מרובעים.\nדוגמה (אם מתחילים מפריים 19): 19.\nהרהור: עוד הפרעה...\n[קצין במדים יושב במשרד מפואר ומביט בטלפון המצלצל]."
            ...
            prompt = f"{preamble}{full_context}לפניך תקציר של סיפור קומיקס. עליך לחלק אותו באופן הגיוני ל-{project.chapters or 18} פרקים. ודא שכל פרק מכיל התפתחות עלילתית משמעותית ומסתיים בנקודת מתח או שיא קטן. הצג את התוצאה כתקציר מחודש, מחולק עם כותרות ברורות לכל פרק (למשל 'פרק 1: ...').\n\nהתקציר המלא:\n{synopsis_text_content}"
...
def create_object(project_id: int, name: str = Form(...), description: str = Form(...), style: str = Form("")):
    ...
    raw_prompt = f"{style_prefix}A single character reference image named '{name}'. {description}. Centered on a plain white background, full body shot, no shadows or other elements."
...
def create_image(project_id: int, desc: str = Form(...), style: str = Form(""), scene_label: str = Form(""), source_image_id: Optional[int] = Form(None)):
    with Session(engine) as session:
        try:
            ...
            raw_prompt_text = f"{style_prefix}A full scene image. Description: {english_desc}"
